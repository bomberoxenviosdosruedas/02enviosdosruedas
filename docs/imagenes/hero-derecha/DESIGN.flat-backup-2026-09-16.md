...
- **Dioramas 3D de Ciudad Miniatura:** la imagen de hero es un render 3D isométrico moderno —baldosa de mapa con canto biselado, manzanas azules mate, ruta amarilla emisiva, pin facetado y scooter de juguete con top box amarilla— recortado sobre transparente y montado en la tarjeta tilt (§9.1). Es la "firma volumétrica" de la marca.
...
- **Estructura:** `grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-12 items-center` con **7/5** — columna izquierda (`lg:col-span-7`) para titular + promesa + CTA; columna derecha (`lg:col-span-5`) para la tarjeta tilt 3D (`FloatTiltCard`) —con su **Hero Card Media** (render 3D isométrico de diorama, transparente, en la cabecera de la tarjeta, §9.1)— o la foto del courier. Sobre fondo `brand-blue-700` con glow radial blanco centrado y blooms de amarillo al 10–18% con `blur(80–130px)`. Nunca centrado en desktop (`text-center` solo `< lg`).
...
- **Generación de imágenes (Nano Banana):** fotografía/escenas según la Estructura 1/2 de `AGENTS.md` (§Prompts de Imágenes; `docs/directrices_imagenes.md` aún no existe en el repo), referencias en `docs/imagenes/`, paleta `#00277C · #FFEC01 · #0636A5`, entorno MDQ. **Ilustraciones de hero:** exclusivamente según §9.1 y `docs/imagenes/hero-derecha/PROMPTS.md`.
- **Dos carriles visuales, sin mezcla:** _fotografía real_ (secciones, imagen inline del titular, zig-zag) e _ilustración flat vector minimal corporate_ (Hero Card Media). Prohibido render 3D, clay o isométrico realista junto a la ilustración flat en la misma pantalla; los iconos Lucide de UI no cuentan como ilustración.

### 9.1 Hero Card Media — Ilustraciones Vectoriales de la Columna Derecha

Ilustración **flat vector minimal corporate** con fondo transparente que vive dentro de la tarjeta de la columna derecha del hero (o en modo standalone en Contacto). Formas geométricas puras, trazo redondeado uniforme de 4px, sombreado sutil en dos tonos y paleta corporativa estricta. Complementa la tarjeta: muestra lo que el texto no muestra (ruta directa, consolidación, picking QR, equipo propio, despacho humano). **Nunca** repite cifras, precios, direcciones ni horarios que la tarjeta ya renderiza.
...
**Fuente de verdad de prompts:** `docs/imagenes/hero-derecha/PROMPTS.md` (catálogo por slug + variantes B). **Pipeline:** `python docs/imagenes/hero-derecha/generate.py [slug ...]` → crudo chroma en `docs/imagenes/hero-derecha/chroma/<slug>.png` → recorte alfa, despill de bordes y optimización en `public/img/heroes/<slug>.{png,webp}`.

**Estructura de prompt obligatoria** (inglés, un bloque por línea):

```text
[Subject: narrativa visual geométrica complementaria al componente] + [Style: flat vector minimal corporate illustration, geometric shapes, clean flat fills, subtle two-tone shading, consistent 4px rounded stroke weight] + [Palette: fórmula cromática según superficie destino] + [Lighting: even studio lighting, uniform and soft, no dramatic shadows, no glow effects] + [Composition: centered composition, aspect ratio exacto, margen de seguridad >= 10%] + [Quality: 8k ultra-detailed, crisp razor-sharp vector edges] + [Background: solid flat uniform chroma key magenta #FF00FF] + [Negative: base negativa canónica]
```
...
**Superficie destino → regla de contraste y siluetas** (define la línea `[Palette]`):

| Superficie | Fondo real                                              | Siluetas exteriores               | Solo detalles internos / sombreado | Regla de contraste                                                       |
| ---------- | ------------------------------------------------------- | --------------------------------- | ---------------------------------- | ------------------------------------------------------------------------ |
| `dark`     | Tarjeta `brand-blue-950` / `#052C87` / `brand-blue-900` | `#FFFFFF` · `#E6EEFE` · `#0950F6` | `#0636A5` · `#00277C`              | Siluetas externas claras obligatorias para recortar sobre el azul noche. |
| `light`    | Tarjeta `bg-white` (LowCost, Flex, Emprendedores)       | `#0636A5` · `#00277C`             | `#0950F6` · `#E6EEFE` · `#FFFFFF`  | Siluetas externas oscuras para contraste nítido contra el blanco.        |
| `bright`   | Sección `brand-blue-500` sin tarjeta (Contacto)         | `#FFFFFF` · `#00277C`             | `#0950F6` · `#E6EEFE`              | Equilibrio entre blanco y navy sobre el azul saturado.                   |

Amarillo `#FFEC01` = único acento (cajones de moto, rutas activas, cintas de sellado, medallas), **≤ 15%** del sujeto (coherente con "un solo acento amarillo grande" de §4). Sin gris, carbón, degradados ni cromo.

**Estilo inquebrantable:** ilustración vectorial plana corporativa, trazo redondeado uniforme de 4px, sombreado sutil en dos tonos, figuras humanas sin rasgos faciales (siluetas estilizadas con casco/uniforme). Anclas MDQ reconocibles por geometría (rambla, olas atlánticas, lobos marinos, faro de Punta Mogotes, casas bajas). Sin texto, números, logos ni marcas externas (Mercado Libre, WhatsApp, Instagram, TikTok, Google) dentro de la imagen.

**Reglas de recorte (chroma key):** fondo `#FF00FF` plano uniforme borde a borde; alternativo `#00B140` si el magenta contamina bordes celestes. Prohibido: glow, halos, semitransparencias, degradados, líneas < 3px, sombras proyectadas sobre el fondo, piso que llegue a los bordes. Margen vacío ≥ 10% en cada lado.
...
- **Sin superposición:** la ilustración ocupa su propio bloque en el flujo; nunca `absolute` detrás o encima de texto de la tarjeta (§10.13).
- **Mobile (`< lg`):** se mantiene dentro de la tarjeta con `max-w-[320px]`; si la tarjeta se oculta en mobile, la ilustración se oculta con ella.
...
**Imagen de hero** 20. Hero Card Media con texto, cifras, logos o marcas externas renderizadas (incluido el logo de marca embebido en el pin); render que repite los datos de la tarjeta; ilustración flat vector o clip-art mezclada con renders 3D; superficies grises, carbón, cromo o dorado; fondo no transparente, halo o tinte magenta; glow que sale del objeto o sombras sobre el fondo; volúmenes navy dominantes sobre tarjeta oscura; más de un acento amarillo dominante; `object-cover` sobre el render; `drop-shadow` gris (§9.1).
...
| Hero Card Media (§9.1)                   | Solo Home tiene render (`/card_mapa.webp`): estilo 3D correcto, pero con texto "ENVIOS DosRuedas MDQ", logo embebido y cara gris en el pin (§10.20). Los otros 10 heroes no tienen slot. Assets nuevos aún no generados (`GEMINI_API_KEY` sin cuota de imagen). | Generar con `docs/imagenes/hero-derecha/generate.py`, reemplazar `card_mapa.webp` por `home.webp` e incorporar el slot `card-media` con `translateZ(40px)` en cada hero. |
...
- **Hero:** asymmetric 7/5 grid on `#0636A5` with radial white glow + yellow blooms (10–18%, blur 80–130px); `min-h-[90dvh]`; left column = badge + 2–3 line Anton headline (one knockout) + one-paragraph promise + ONE primary CTA + 3 factual chips; right column = 3D tilt card (with Hero Card Media: transparent modern 3D isometric miniature-city diorama render in the card header at `translateZ(40px)` with navy-tinted drop-shadow, 1:1 on home / 4:3 elsewhere, `next/image fill object-contain`, `alt=""`, complements card data and never repeats it) or real courier photo. Optional inline photo between headline words (rounded, x-height, stacks below on mobile). No centered hero on desktop, no scroll prompts, no overlapping.
...
- **Hero renders (Hero Card Media):** modern 3D isometric miniature diorama renders only (style canon `public/card_mapa.webp` minus its text/logo/grey), built from the DosRuedas 3D Kit — rounded map-tile base with bevelled blue edge, matte blue city blocks, glossy emissive yellow route tube, faceted blue pin with yellow rim, toy-like blue scooter with yellow top box, pale-blue parcels with yellow tape, faceless vinyl-toy couriers. Prompts from `docs/imagenes/hero-derecha/PROMPTS.md` with structure `[Subject] [Style: 3D isometric diorama] [Palette] [Lighting: soft studio three-point] [Composition: isometric 30°, centered] [Quality: 8k ultra-detailed] [Background: unlit chroma #FF00FF] [Negative]`. Materials by target surface — dark card: bright/sky-blue volumes with pale-blue tops; white card: egyptian-blue volumes with navy sides; bright-blue section: white/pale-blue volumes. Yellow `#FFEC01` single accent ≤ 15%. No text, numbers, logos, external brands, faces, grey/chrome/gold, glow outside objects or shadows on background. Never mix with flat vector illustration on the same screen.
- **Language & Tone:**
