...
Fuentes: `F2-1-design-critique.md` (DC-01 a DC-34, D-1 a D-12), `F2-2-accesibilidad.md` (A11Y-01 a A11Y-15), `F2-3-ux-copy.md` (COPY-01 a COPY-10), `F2-4-brand-review.md` (MARCA-01 a MARCA-03 y tabla de afirmaciones), `F3-1-campaign-plan.md` (CAMP-01 a CAMP-20), `F3-2-contenidos.md` (CONT-01 a CONT-18), `F6-visibilidad-ia.md` (GEO-01 a GEO-05), `F7-reputacion.md` (REP-01 a REP-03). No se revisaron pull requests abiertos del repo real (esta sesión trabaja sobre un clon local sin acceso a GitHub en vivo) — si hay PRs en curso, hay que cruzarlos contra este backlog antes de empezar el Sprint 1, no después.

**Actualización tras Fases 6-7 (GEO y Reputación):** se agregan `BL-40` a `BL-42` al final de la tabla (§2), sin renumerar los ítems existentes, porque `F4-1-specs/` y `F4-2-prompts/` del Sprint 1 ya se emitieron contra los números `BL-01` a `BL-19` — renumerar rompería esa referencia. `GEO-02` (horario incorrecto también presente en el JSON-LD duplicado por página) no genera un ID nuevo: confirma y refuerza `BL-02`, que ya lo cubre. Los hallazgos `REP-01` a `REP-03` de la Fase 7 tampoco generan IDs nuevos: `REP-01` confirma `BL-16` (ya existente), y `REP-02`/`REP-03` son una pregunta al dueño y un ajuste de contenido de `F3-1-campaign-plan.md` respectivamente, no cambios de código — ver `F7-reputacion.md` §9.
...
| BL-02 | Metadata global: `title.template` sin marca duplicada, quitar `keywords`, horarios reales en JSON-LD, `sameAs`/`logo`/`hasMap`/`image` | D-9, CAMP-04, MARCA-01, GEO-02 | `src/app/layout.tsx`, `page.tsx` de cada ruta | Bug / SEO técnico | Alto | Bajo | Bajo | — | **Sí** — horario real (F2-4 P5) |
...
| BL-40 | Sección nueva en `llms.txt` orientada a clientes, con URLs públicas reales de servicios/cotizadores/FAQ (hoy el archivo solo documenta el repo de código para agentes programadores) | GEO-01 | `llms.txt` | SEO técnico / GEO | Alto | Bajo | Bajo | — | No |
| BL-41 | `BreadcrumbList` en JSON-LD de páginas de servicio y FAQ (el componente ya lo soporta; no se usa en ninguna página real) | GEO-03 | Páginas de servicio, `nosotros/preguntas-frecuentes`, decidir si vía `SchemaMarkup.tsx` o inline (ver BL-39) | SEO técnico | Bajo-Medio | Bajo | Bajo | BL-39 | No |
| BL-42 | Cabecera `X-Robots-Tag: noindex` condicionada a `VERCEL_ENV !== 'production'`, para que los deployments de preview de Vercel (ej. `02enviosdosruedas.vercel.app`) no queden indexables en paralelo al dominio real | GEO-04 | `next.config.ts` | SEO técnico | Bajo | Bajo | Bajo | — | No |
...
| **9** (agregado tras Fases 6-7) | BL-40, BL-42, BL-41 | `BL-40` y `BL-42` son SEO técnico de bajo esfuerzo y buen impacto — por la regla de orden de §1 (bugs de indexación primero) calificarían para un sprint temprano tipo Sprint 1-2, pero se numeran al final para no interferir con los sprints ya en curso o ya especificados; quien programe el orden real puede adelantarlos. `BL-41` depende de la misma decisión de `BL-39` (¿centralizar en `SchemaMarkup.tsx` o seguir con JSON-LD inline por página?), así que conviene resolverlos juntos. |
...
| BL-41 (`areaServed`, no bloquea el ítem en sí pero sí su alcance) | ¿La cobertura real llega a Batán/Partido de General Pueyrredón, o es correcto que hoy solo cubra el ejido urbano de Mar del Plata como dice el FAQ? | `F6-visibilidad-ia.md` GEO-05 |
