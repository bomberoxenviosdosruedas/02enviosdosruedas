...
export default function FaqSection() {
  ...
  return (
    <section id="faqs" className="py-14 bg-brand-white-50 px-4 sm:px-6 lg:px-8 border-b border-brand-blue-50">
      <div className="max-w-4xl mx-auto space-y-10">
        
        {/* Title */}
        <div className="text-center space-y-2">
          <div className="inline-flex h-9 w-9 rounded-xl bg-brand-blue-50 text-brand-blue-500 items-center justify-center font-bold">
            ?
          </div>
          <h2 className="text-3xl font-black font-display tracking-tight text-brand-ink">
            Preguntas <span className="text-brand-blue-400">Frecuentes</span>
          </h2>
          <p className="text-brand-blue-400 text-xs sm:text-sm max-w-lg mx-auto">
            Despejá tus dudas sobre seguros, demoras climáticas, recargos y resguardo de mercaderías.
          </p>
        </div>

        {/* Accordion list */}
        <div className="bg-white border border-brand-blue-100 rounded-2xl overflow-hidden shadow-sm divide-y divide-brand-blue-50">
          {FAQS_POOL.map((item) => {
            const isOpen = openKey === item.k;
            return (
              <div key={item.k} className="transition-colors hover:bg-brand-blue-200/40">
                <button
                ...
                >
                  <span className="font-bold text-brand-ink text-xs sm:text-sm flex items-center gap-2">
                    <HelpCircle className="h-4 w-4 text-brand-blue-400 flex-shrink-0" />
                    {item.q}
                  </span>
                  {isOpen ? (
                    <ChevronUp className="h-4 w-4 text-brand-blue-300" />
                  ) : (
                    <ChevronDown className="h-4 w-4 text-brand-blue-300" />
                  )}
                </button>

                {isOpen && (
                  <div className="px-6 pb-5 text-xs text-brand-blue-500 leading-relaxed font-normal bg-brand-white-50/50">
        ...
        <div className="bg-brand-blue-700 text-brand-blue-200 p-5 rounded-2xl flex flex-col sm:flex-row items-center gap-4 border border-brand-blue-700">
          <div className="h-10 w-10 rounded-xl bg-brand-blue-400/10 text-brand-blue-300 flex items-center justify-center border border-brand-blue-400/20 flex-shrink-0">
            <Lock className="h-5 w-5" />
          </div>
          <div className="space-y-1 text-center sm:text-left">
            <h4 className="font-bold text-white text-xs uppercase tracking-wide">Cumplimiento Normativo ARS</h4>
            <p className="text-[10px] text-brand-blue-500 leading-relaxed font-normal">
