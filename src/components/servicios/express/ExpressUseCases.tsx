    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-2xl mx-auto mb-16 space-y-4">
          <span className="-rotate-1 inline-block px-4 py-1.5 bg-[#FFEC01] text-[#0950F6] rounded-full text-xs font-subheading font-bold uppercase tracking-widest shadow-sm">
            CASOS DE USO REALES
          </span>
          ...
          <p className="text-[#0950F6]/80 text-base sm:text-lg font-sans max-w-lg mx-auto leading-relaxed">
          ...
          {cases.map((useCase, idx) => {
            ...
            return (
              <div key={useCase.title}>
                <DoubleBezelCard>
                  <div className="space-y-6 h-full flex flex-col justify-between text-left relative overflow-hidden">
                    <Icon className="absolute -bottom-6 -right-6 h-32 w-32 text-[#0950F6]/[0.05] pointer-events-none select-none" />

                    {/* Icon & Badge Header */}
                    <div className="flex justify-between items-center relative z-10">
                      <div className="p-3 rounded-xl border flex items-center justify-center shrink-0 bg-[#0950F6] text-[#FFEC01] border-[#0950F6] shadow-sm">
                        <Icon className="h-6 w-6 shrink-0" />
                      </div>
                      <span className="text-[10px] font-subheading font-bold uppercase tracking-wider px-2.5 py-1 rounded-full border bg-[#E6EEFE] text-[#0950F6] border-[#D6E4FE]">
                        {useCase.badge}
                      </span>
                    </div>
                    ...
                    <div className="space-y-2 relative z-10">
                      <h3 className="text-2xl font-display uppercase tracking-wide leading-tight text-[#0950F6]">
                        {useCase.title}
                      </h3>
                      <p className="text-sm font-sans leading-relaxed text-[#0950F6]/80">
                        {useCase.desc}
                      </p>
                    </div>
                    ...
                    <button
                      ...
                      className="w-full py-3 px-4 rounded-xl text-xs font-bold tracking-wider uppercase font-subheading flex items-center justify-between border transition-all cursor-pointer min-h-[44px] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0950F6] relative z-10 bg-[#FFEC01] text-[#0950F6] border-[#FFEC01] hover:bg-[#FFEC01]/90 shadow-glow-yellow"
                    >
                    ...
                    <AnimatePresence initial={false}>
                      {isOpen && (
                        ...
                        >
                          <div className="space-y-3 pt-4 border-t border-[#D6E4FE]">
                            <p className="text-[10px] font-bold tracking-widest text-[#0950F6] uppercase font-mono">
                            ...
                            <ul className="space-y-2">
                              {useCase.examples.map((ex, exIdx) => (
                                <motion.li
                                  ...
                                  className="flex items-start gap-2 text-xs text-[#0950F6]/90 font-sans"
                                >
                                  <CheckCircle2 className="h-4 w-4 text-[#0950F6] shrink-0 mt-0.5" />
